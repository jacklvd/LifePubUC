...
const agendaItemSchema = z.object({
  ...
  startTime: z.string().regex(/^(0[1-9]|1[0-2]):[0-5][0-9] (AM|PM)$/, 'Invalid start time format (HH:mm AM/PM)'),
  endTime: z.string().regex(/^(0[1-9]|1[0-2]):[0-5][0-9] (AM|PM)$/, 'Invalid end time format (HH:mm AM/PM)'),
}).refine((data) => {
...
const agendaSchema = z.object({
  id: z.string(),
  title: z.string().min(3, 'Agenda title must be at least 3 characters.'),
  items: z.array(agendaItemSchema).min(1, 'Agenda must contain at least one item'),
});

const faqSchema = z.object({
  question: z.string().min(1, 'Question is required'),
  answer: z.string().min(1, 'Answer is required').max(400, 'Answer is too long'),
});
...
export const eventSchema = z.object({
  ...
  summary: z.string().min(3, 'Summary must be at least 3 characters long.').max(200, 'Summary must be at most 200 characters long.'),
  ...
  startTime: z.string().regex(/^(0[1-9]|1[0-2]):[0-5][0-9] (AM|PM)$/, 'Invalid time format (HH:mm AM/PM)'),
  endTime: z.string().regex(/^(0[1-9]|1[0-2]):[0-5][0-9] (AM|PM)$/, 'Invalid time format (HH:mm AM/PM)'),
