...
export async function getEventTickets(eventId: string): Promise<any> {
  try {
    console.log(`Fetching tickets from: ${API_URL}/api/events/${eventId}/tickets`);
    
    const response = await axios.get(`${API_URL}/api/events/${eventId}/tickets`, {
      headers: {
...
export async function createTicket(eventId: string, ticketData: TicketFormData): Promise<Ticket> {
  try {
    // console.log(`Creating ticket at: ${API_URL}/api/events/${eventId}/tickets`);
    // console.log('Ticket data:', ticketData);
    
    const response = await axios.post(`${API_URL}/api/events/${eventId}/tickets`, ticketData, {
      headers: {
        'Content-Type': 'application/json',
      },
    });
...
export async function updateTicket(eventId: string, ticketId: string, ticketData: Partial<TicketFormData>): Promise<Ticket> {
  try {
...
export async function updateEventTotalCapacity(eventId: string, totalCapacity: number): Promise<void> {
  try {
