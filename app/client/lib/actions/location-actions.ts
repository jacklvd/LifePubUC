 ...
 */
export async function matchesLocation(eventLocation: string, filterLocation: string): Promise<boolean> {
...
export async function getEventLocations(): Promise<string[]> {
  try {
    ...
    const response = await axios.get(`${API_BASE_URL}/api/events/all-events?limit=300&status=on sale`)
    ...
    // Extract locations 
    const rawLocations = response.data.events.map((event: Event) => event.location || '')
 ...
 */
export async function getPopularLocations(limit: number = 5): Promise<string[]> {
  try {
    // Fetch a large number of events to get location data
    const response = await axios.get(`${API_BASE_URL}/api/events/all-events?limit=200&status=on sale`)
    
    if (response.status !== 200) {
      throw new Error(`Failed to fetch events for popular locations. Status: ${response.status}`)
    }
 ...
 */
export async function getLocationsWithEvents(timeframe?: string): Promise<string[]> {
  try {
    ...
    if (timeframe) {
      queryParams.append('date', timeframe === 'next-week' ? 'upcoming' : timeframe)
    }
    ...
    if (response.status !== 200) {
      throw new Error(`Failed to fetch events for locations. Status: ${response.status}`)
    }
