...
import { getEventTickets, createTicket, updateTicket, deleteTicket } from '@/lib/actions/ticket-actions'
import { EventStep } from '@/components/event-ui/event-leftside-bar' // Import the EventStep type
import { format } from 'date-fns'

export const useTicketManagement = (eventId: string, markStepCompleted: (step: EventStep) => void) => {
    ...
    const [saleStartDate, setSaleStartDate] = useState<Date | undefined>(new Date())
...
const resetForm = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      const futureDate = parsedEventDate 
        ? new Date(Math.min(parsedEventDate.getTime() - 86400000, now + 2592000000))
...
const isEndDateDisabled = useCallback((date: Date) => {
  ...
  const handleAddTicket = useCallback(async () => {
    try {
      console.log('Add ticket submission with dates:', saleStartDate, saleEndDate);
    ...
    eventId, parsedEventDate, ticketName, ticketCapacity, ticketType, ticketPrice,
...
const handleUpdateTicket = useCallback(async () => {
    try {
      ...
      const updatedTicket = await updateTicket(eventId, currentTicket.id, ticketData)
    ...
    currentTicket, eventId, event, ticketName, ticketCapacity, ticketType, ticketPrice,
    saleStartDate, saleEndDate, startTime, endTime, minPerOrder, maxPerOrder, tickets
  ])
  ...
  const handleDeleteTicket = useCallback(async () => {
    try {
      ...
      // Update tickets list
      const filteredTickets = tickets.filter(ticket => ticket.id !== currentTicket.id)
  ...
  const formatTicketDate = useMemo(() => (date: Date) => {
    return format(new Date(date), 'MMM dd, yyyy');
  }, []);

  const formatEventDate = useMemo(() => (date: string | Date | undefined): string => {
    if (!date) return '';
