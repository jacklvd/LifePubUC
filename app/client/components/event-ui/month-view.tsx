...
import { getWeekDayNames, getDaysInMonth, getPrevMonthDays, getNextMonthDays, isPastDate, getEventColor, formatTime } from '@/lib/utils'
...
const DayCell = React.memo(({
    ...
    const onEventClick = useCallback((event: Event, e: React.MouseEvent) => {
        e.stopPropagation()
...
export const MonthView: React.FC<MonthViewProps> = React.memo(({ date, getEventsForDate, handleEventClick }) => {
    ...
    const currentMonthDays = useMemo(() => getDaysInMonth(date.year, date.month), [date.year, date.month])
    const prevMonthDays = useMemo(() => getPrevMonthDays(date.year, date.month), [date.year, date.month])
    const nextMonthDays = useMemo(() => getNextMonthDays(date.year, date.month), [date.year, date.month])
    ...
    const eventsByDate = useMemo(() => {
        ...
        return map
    }, [date.year, date.month, prevMonthDays, currentMonthDays, nextMonthDays, getEventsForDate])
