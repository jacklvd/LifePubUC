  ...
  return (
    <>
      {hours.map((hour, hourIndex) => (
        <div
          key={hourIndex}
          className="w-[70px] h-14 border-b border-r text-xs text-right pr-2 pt-1 text-gray-500 flex-shrink-0"
        >
          {hour}
        </div>
      ))}
...
export const WeekView: React.FC<WeekViewProps> = React.memo(
  ({ weekDays, weekContainerRef, getEventsForDate, handleEventClick }) => {
    const today = useMemo(() => {
      const now = new Date()
    ...
    return (
      ...
      >
        ...
        <div className="flex border-b sticky top-0 bg-white-100 z-20">
          ...
          {/* Day headers */}
          <div className="flex flex-1">{dayHeaders}</div>
        </div>
