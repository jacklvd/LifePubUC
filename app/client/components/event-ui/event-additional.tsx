...
const EventAdditional: React.FC<AdditionalProps> = ({
    ...
    const [highlightType, setHighlightType] = useState<'age' | 'door' | 'parking' | ''>('')
    ...
    const handleSaveHighlight = () => {
        if (highlightType === 'age') {
            ...
            onUpdateHighlights({ ...highlights, doorTime: `${doorTimeValue} ${doorTimeUnit}` })
    ...
    return (
        <>
                ...
                <div className="relative rounded-md overflow-hidden border bg-white-100 p-6">
                    ...
                    <div className="mb-6">
                        ...
                        <div className="flex flex-wrap gap-2">
                            ...
                            {highlights.ageRestriction && (
                                <Badge variant="secondary" className="cursor-pointer" onClick={() => handleOpenHighlightsDialog('age')}>
                                    {highlights.ageRestriction === 'all' ? 'All ages allowed' :
                                        highlights.ageRestriction === 'restricted' ? 'Age restriction applies' :
                            ...
                            {highlights.doorTime && (
                                <Badge variant="secondary" className="cursor-pointer" onClick={() => handleOpenHighlightsDialog('door')}>
                            ...
                            {highlights.parkingInfo && (
                                <Badge variant="secondary" className="cursor-pointer" onClick={() => handleOpenHighlightsDialog('parking')}>
                                    {highlights.parkingInfo === 'free' ? 'Free parking' :
                                        highlights.parkingInfo === 'paid' ? 'Paid parking' :
                                            'No parking options'}
                                </Badge>
                            )}
                        </div>
                    ...
                    <div>
                        ...
                        {faqs.length === 0 && (
                            <div className="border rounded-md p-4 mb-3 bg-blue-50 flex items-center">
                                ...
                                <div>
                                    <p className="text-sm font-medium">Boost your event visibility</p>
                                    <p className="text-sm text-gray-600">
                                        Events with FAQs have 8% more organic traffic. Help attendees find answers easily.
                                    </p>
                                </div>
                            </div>
                        )}
                ...
                <DialogContent className="sm:max-w-md bg-white-100">
                    <DialogHeader>
                        <DialogTitle>
                            ...
                            {highlightType === 'door' && 'What time can attendees check in before the event?'}
                    ...
                    <div className="py-4">
                        {highlightType === 'age' && (
                            <RadioGroup value={ageRestriction} onValueChange={setAgeRestriction} className="flex flex-col gap-4">
                                ...
                                <div className="flex items-center space-x-2 border p-4 rounded-md">
                                    ...
                                    <Label htmlFor="restricted">There&apos;s an age restriction</Label>
                                </div>
                                <div className="flex items-center space-x-2 border p-4 rounded-md">
                        ...
                        {highlightType === 'parking' && (
                            <RadioGroup value={parkingType} onValueChange={setParkingType} className="flex flex-col gap-4">
                                <div className="flex items-center space-x-2 border p-4 rounded-md">
                    ...
                    <DialogFooter>
                        <Button onClick={handleSaveHighlight} className="bg-red-500 text-white hover:bg-red-600">
                            Add to event
                        </Button>
                    </DialogFooter>
                </DialogContent>
            ...
            <Dialog open={showFaqDialog} onOpenChange={setShowFaqDialog}>
                <DialogContent className="sm:max-w-md bg-white-100">
                    <DialogHeader>
                        <DialogTitle>{editingFaqIndex !== null ? 'Edit question' : 'Add question'}</DialogTitle>
                    </DialogHeader>
                    ...
                    <div className="py-4 space-y-4">
                        <div className="space-y-2">
                            ...
                            {currentFaq.question.trim() === '' && (
                                <p className="text-red-500 text-sm">A question is required for this section</p>
                            )}
                        </div>
                        ...
                        <div className="space-y-2">
                            ...
                            {currentFaq.answer.trim() === '' && (
                                <p className="text-red-500 text-sm">An answer is required for this section</p>
                            )}
                            <div className="flex justify-between text-sm text-gray-500">
                                <span className={currentFaq.answer.length > 280 ? "text-amber-600" : ""}>
                                    {currentFaq.answer.length > 280 ? "Keep it concise for better readability" : ""}
                                </span>
                                <span className={currentFaq.answer.length > 280 ? "text-amber-600 font-medium" : ""}>
