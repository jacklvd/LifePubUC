...
const EventDateTimePicker = ({
  ...
  const isTimeInPast = useCallback((hour: string, minute: string, period: string) => {
          ...
          <div className="flex space-x-2">
            ...
            >
              ...
              <SelectContent className="bg-white-100">
                <SelectItem
                  value="AM"
                  disabled={isTodayDate(date) && startHour !== '12' && new Date().getHours() >= 12}
                >
                  AM
                </SelectItem>
          ...
          <div className="flex space-x-2">
              ...
              <SelectContent className="bg-white-100">
                {hours.map((hour) => (
                  <SelectItem
                    key={`end-hour-${hour}`}
                    value={hour}
                    disabled={isTodayDate(date) && isTimeInPast(hour, endMinute, endPeriod)}
                  >
                    {hour}
                  </SelectItem>
                ))}
              ...
              <SelectContent className="bg-white-100">
                {minutes.map((minute) => (
                  <SelectItem
                    ...
                    value={minute}
                    disabled={isTodayDate(date) && isTimeInPast(endHour, minute, endPeriod)}
                  >
                    {minute}
                  </SelectItem>
                ))}
            ...
            >
              ...
              <SelectContent className="bg-white-100">
                <SelectItem
                  value="AM"
                  disabled={isTodayDate(date) && endHour !== '12' && new Date().getHours() >= 12}
                >
                  AM
                </SelectItem>
                <SelectItem value="PM">PM</SelectItem>
