...
const EventAgenda = ({ agendas, setAgendas, eventStartTime, eventEndTime, errors }: EventAgendaProps) => {
    const [activeAgenda, setActiveAgenda] = useState('welcome');
    const [showAgendaForm, setShowAgendaForm] = useState(false);
    const [newAgendaTitle, setNewAgendaTitle] = useState('');
    const [showTimeDropdown, setShowTimeDropdown] = useState(false);
    const [selectedTimeField, setSelectedTimeField] = useState<{ field: 'startTime' | 'endTime'; itemId: string } | null>(null);
    const [editingItem, setEditingItem] = useState<string | null>(null);
    const [formErrors, setFormErrors] = useState<Record<string, string>>({});
    const [menuOpenForAgenda, setMenuOpenForAgenda] = useState<string | null>(null);
    ...
    const generateTimeSlots = (startTime?: string, endTime?: string) => {
        ...
        if (!startTime || !endTime || startTime.trim() === "H" || endTime.trim() === "H") {
            console.warn("🚨 Incomplete start or end time. Skipping time slot generation.");
            return [];
        }
        ...
        let { hour: currentHour, minute: currentMinute, period: currentPeriod } = start;
        const { hour: endHour, minute: endMinute, period: endPeriod } = end;
        const timeSlots = [];

        while (
            (currentPeriod === endPeriod && (currentHour < endHour || (currentHour === endHour && currentMinute <= endMinute))) ||
            (currentPeriod === "AM" && endPeriod === "PM")
        ) {
            timeSlots.push(`${currentHour}:${currentMinute.toString().padStart(2, '0')} ${currentPeriod}`);
    ...
    useEffect(() => {
        const handleClickOutside = (e: MouseEvent) => {
            if (editingRef.current && !editingRef.current.contains(e.target as Node)) {
                ...
                const currentItem = currentAgenda?.items.find(item => item.id === editingItem);
    ...
    const handleDeleteAgenda = (agendaId: string) => {
        ...
        if (activeAgenda === agendaId) {
            const remainingAgendas = agendas.filter(agenda => agenda.id !== agendaId);
    ...
    const handleCreateNewAgenda = () => {
        ...
        setAgendas(agendas.map(agenda => ({ ...agenda, active: false })).concat(newAgenda));
        setActiveAgenda(agendaId);
        setNewAgendaTitle('');
    ...
    const handleTimeClick = (fieldId: 'startTime' | 'endTime', itemId: string) => {
    ...
    const handleDeleteItem = (itemId: string) => {
        ...
        if (currentAgenda.items.length === 1) {
            ...
            setFormErrors({ ...formErrors, agenda: "You must have at least one agenda item or delete the entire agenda" });
            return;
        }
    ...
    const renderAgendaItem = (item: any) => {
        ...
        if (isEditing || item.isNew) {
            return (
                <div key={item.id} ref={editingRef} className="agenda-editing mt-4 bg-white-100 p-4 rounded-md border">
                    <div className="mb-4">
                        <Input
                    ...
                    <div className="flex gap-4 mb-4">
                        <div className="flex-1 relative">
                            <Button onClick={() => handleTimeClick('startTime', item.id)} className="flex items-center border rounded-md p-2 w-full text-black bg-white-100 hover:bg-slate-900 hover:text-white-100">
                                <Icon name="Clock" className="h-4 w-4 mr-2" />
                                {item.startTime || 'Start time'}
                            </Button>

                            {/* Start Time Dropdown */}
                            {selectedTimeField?.field === 'startTime' && selectedTimeField?.itemId === item.id && openDropdownId === item.id && (
                        ...
                        <div className="flex-1 relative font-light">
                            <Button onClick={() => handleTimeClick('endTime', item.id)} className="flex items-center border rounded-md p-2 w-full text-black bg-white-100 hover:bg-slate-900 hover:text-white-100">
                                <Icon name="Clock" className="h-4 w-4 mr-2" />
                                {item.endTime || 'End time'}
                            </Button>

                            {/* End Time Dropdown */}
                            {selectedTimeField?.field === 'endTime' && selectedTimeField?.itemId === item.id && openDropdownId === item.id && (
                                <div className="absolute bg-white-100 border rounded-md shadow-lg max-h-60 overflow-y-auto mt-1 z-20">
                                    {timeSlots.map(time => (
                                        <div
                                            key={time}
                    ...
                    <div className="mb-4">
                        <Input
                            ...
                            value={item.host}
                            onChange={(e) => handleInputChange(item.id, 'host', e.target.value)}
                            className="w-full"
                        />
                    </div>
                    ...
                    <div className="flex justify-between items-center">
                        <button
                            ...
                            onClick={() => !isDeleteButtonDisabled && handleDeleteItem(item.id)}
                            disabled={isDeleteButtonDisabled}
                            title={!item.title.trim() ? "Please enter a title before deleting" :
                                (agendas.find(a => a.id === activeAgenda)?.items.length === 1 ?
        ...
        return (
            <div key={item.id} className="mt-4 bg-lime-50 p-4 rounded-md border-l-4 border-l-lime-300">
                <div className="flex justify-between items-center">
                    <div>
                        <p className="text-gray-600">{item.startTime}{item.endTime ? ` - ${item.endTime}` : ''}</p>
            ...
            <div className="relative rounded-md overflow-hidden border bg-white-100 p-6">
                ...
                {(globalAgendaError || formAgendaError) && (
                    <div className="bg-red-50 border-l-4 border-red-500 p-4 mb-4">
                        <p className="text-red-700">{globalAgendaError || formAgendaError}</p>
                    </div>
                )}
                <p className="text-gray-600 mb-4">
                    Add an itinerary, schedule, or lineup to your event. You can include a time, a description of what will happen, and
                    who will host or perform during the event. (Ex. Speaker, performer, artist, guide, etc.) If your event has multiple
                    dates, you can add a second agenda.
                </p>

                {/* Agenda Tabs */}
                <div className="flex items-center mb-6 border-b">
                    {agendas.map(agenda => (
                        <div key={agenda.id} className={`flex items-center mr-4 pb-2 ${agenda.active ? 'border-b-2 border-blue-500 text-blue-500' : 'text-gray-500'}`}>
                            {renamingAgendaId === agenda.id ? (
                                ...
                                <span className="mr-2 cursor-pointer" onClick={() => handleAgendaTabClick(agenda.id)}>
                                    {agenda.title || 'Untitled agenda'}
                                </span>
                            )}

                            <div className="relative">
                                <button onClick={() => setMenuOpenForAgenda(menuOpenForAgenda === agenda.id ? null : agenda.id)}>
                                    <Icon name="MoreVertical" className="h-4 w-4" />
                                </button>
                                {menuOpenForAgenda === agenda.id && (
                                    <div className="absolute bg-white-100 shadow-lg rounded-md py-2 z-10">
                                        <button className="block w-full px-4 py-1 text-left hover:bg-gray-100" onClick={() => handleRenameAgenda(agenda.id)}>
                                            Rename
                                        </button>
                                        <button className="block w-full px-4 py-1 text-left text-red-500 hover:bg-gray-100" onClick={() => handleDeleteAgenda(agenda.id)}>
                                            Delete
                                        </button>
                                    </div>
                                )}
                ...
                {/* Agenda Items */}
                <div>
                    {activeAgenda && agendas.find(agenda => agenda.id === activeAgenda)?.items.map(item => renderAgendaItem(item))}

                    {activeAgenda && agendas.find(agenda => agenda.id === activeAgenda)?.items.length === 0 && (
                        <div className="text-center p-6 bg-gray-50 rounded-md">
                            <p className="text-gray-500">No agenda items yet. Add your first item below.</p>
                        </div>
                    )}
                </div>
                ...
                {/* Time Dropdown */}
                {showTimeDropdown && (
                    <div className="absolute z-10 bg-white border rounded-md shadow-lg max-h-60 overflow-y-auto" style={{ top: '50%', left: '50%', transform: 'translate(-50%, -50%)' }}>
                        <div className="p-2">
                            {timeSlots.map(time => (
                                <div
                                    key={time}
