...
const EventPreview: React.FC<EventPreviewProps> = ({
    ...
    const renderPreviewContent = () => (
        <div className="space-y-6 overflow-y-auto max-h-full pb-20 px-2 sm:px-0">
                ...
                <CardContent className="p-4 sm:p-6 pt-0 sm:pt-0">
                    {event.tickets && event.tickets.length > 0 ? (
                                ...
                                <div key={ticket.id} className="border rounded-md p-2 sm:p-3">
                                    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2 mb-2">
                                        ...
                                        <Badge className="bg-blue-100 text-blue-800 hover:bg-blue-100 self-start">
                                            <span className="text-xs">{ticket.capacity} available</span>
                                        </Badge>
                                    </div>
                                    <div className="text-xs sm:text-sm text-gray-600">
                                        Sale: {formatDate(ticket.saleStart)} - {formatDate(ticket.saleEnd)}
                                    </div>
                                </div>
                            ))}
                        </div>
                        ...
                        <div className="text-center py-4">
                            <Icon name="Ticket" className="h-6 w-6 sm:h-8 sm:w-8 text-gray-300 mx-auto mb-2" />
                            <p className="text-xs sm:text-sm text-gray-500">No tickets have been created yet.</p>
                        </div>
                    )}
                </CardContent>
            </Card>
            ...
            {/* Highlights */}
            {event.highlights && Object.keys(event.highlights).some(key => !!event.highlights[key]) && (
                <Card>
                    <CardHeader className="p-4 sm:p-6">
                        <CardTitle className="text-base sm:text-lg">Event Highlights</CardTitle>
                    </CardHeader>
                    ...
                    <CardContent className="p-4 sm:p-6 pt-0 sm:pt-0 space-y-4">
                        {event.faqs.map((faq: any, index: number) => (
                            <div key={index} className="border-b pb-3 last:border-0">
                                <h3 className="font-medium mb-1 text-sm sm:text-base">{faq.question}</h3>
