...
const EventCard: React.FC<{ event: Event }> = ({ event }) => {
    ...
    const formattedDate = event.date ? format(new Date(event.date), 'MMM d, yyyy') : 'No date'
    const attendees = event.tickets?.reduce((sum, ticket) => sum + (ticket.sold || 0), 0) || 0
            ...
            <CardContent className="p-0">
                <div className="relative">
                    <Image
                        ...
                        height={220}
                    />
                    <div className={`absolute top-3 right-3 px-2 py-1 rounded-full text-xs font-medium ${statusInfo.className}`}>
                        {statusInfo.label}
                    </div>
                </div>
                <div className="p-4">
    ...
    <Link href="/organization/events/create" passHref>
        <Card className="border-dashed border-2 border-gray-300 hover:border-gray-400 flex items-center justify-center h-full min-h-60 cursor-pointer">
            <CardContent className="p-6 text-center">
                ...
                <h3 className="text-lg font-medium text-gray-800 mb-1">Create New Event</h3>
                <p className="text-sm text-gray-500">Add your next event to start selling tickets</p>
            </CardContent>
        </Card>
    </Link>
)
