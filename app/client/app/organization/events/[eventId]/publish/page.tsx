...
export default function PublishPage({ params }: PublishPageProps) {
    ...
    const fetchEvent = useCallback(async () => {
        try {
            ...
            if (!completedSteps.includes('tickets') && eventData?.tickets?.length > 0) {
    ...
    useEffect(() => {
        if (!loading && !isEditing && !completedSteps.includes('tickets') && !redirectAttempted.current) {
    ...
    const handlePublish = useCallback(async () => {
        ...
        try {
            ...
            if (!event.tickets || event.tickets.length === 0) {
                ...
                const invalidTickets = event.tickets.filter((ticket: { saleEnd: string | number | Date; capacity: number }) => {
                    const saleEnd = new Date(ticket.saleEnd);
                    return ticket.capacity <= 0 || saleEnd > eventDate;
                });

                if (invalidTickets.length > 0) {
                    validationErrors.push('Some tickets have invalid capacity or sale end dates');
                }
            }
                                    ...
                                    <div className="space-y-4">
                                        <div className="flex items-start space-x-3">
                                            <Checkbox
                                                ...
                                                onCheckedChange={(checked) => setDetailsChecked(checked as boolean)}
                                            />
                                            <div>
                                        ...
                                        <div className="flex items-start space-x-3">
                                            <Checkbox
                                                ...
                                                onCheckedChange={(checked) => setTicketsChecked(checked as boolean)}
                                            />
                                            <div>
                                                <label htmlFor="tickets" className="font-medium cursor-pointer">
                                        ...
                                        <div className="flex items-start space-x-3">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-sm text-gray-500">
                                                    By publishing this event, you agree to our terms and conditions
                                                </p>
                                            </div>
                                        </div>
                                    </div>
