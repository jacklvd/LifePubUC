  ...
  return (
    <div className="grid grid-cols-4 sm:grid-cols-4 md:grid-cols-8 gap-4 mb-10">
      {icons.map((category, index) => (
        ...
        >
          <div
            className={`w-14 h-14 md:w-16 md:h-16 rounded-full border transition-all duration-300 flex items-center justify-center mb-2 cursor-pointer
            ${
          ...
          >
            <span
              className={`text-xl md:text-2xl ${activeCategory === index ? 'text-blue-500' : ''}`}
            >
              <Icon name={category.name} />
            </span>
          </div>
          <span
            className={`text-xs md:text-sm text-center font-medium ${activeCategory === index ? 'text-blue-500' : ''}`}
          >
            {category.title}
          </span>
        </div>
