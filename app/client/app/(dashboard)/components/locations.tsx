  ...
  return (
    <div className="flex items-center mb-8 relative">
      ...
      >
        ...
        <Icon
          name="ChevronDown"
          className={`h-4 w-4 ml-1 transition-transform duration-300 ${showLocationDropdown ? 'rotate-180' : ''}`}
        />
      </div>
          ...
          {locations.map((loc, index) => (
            ...
            >
              <div className="flex items-center">
                <MapPin
                  className={`h-4 w-4 mr-2 ${loc === location ? 'text-blue-500' : 'text-gray-400'}`}
                />
                <span
                  className={
                    loc === location ? 'font-medium text-blue-500' : ''
                  }
                >
                  {loc}
                </span>
