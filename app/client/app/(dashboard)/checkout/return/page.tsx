    ...
    const checkoutStatus = async () => {
      try {
        ...
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/payment/session-status?session_id=${sessionId}`,
        )
        const {
          data: { status, customer_email },
        } = await response.json()
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
      <div className="bg-white p-8 rounded-lg shadow-md w-full max-w-md">
        <div className="flex flex-col items-center justify-center space-y-4 text-center">
          ...
          <Link
            href="/"
            className="mt-6 inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-primary-300 hover:bg-primary-500 focus:outline-none focus:ring-2 focus:ring-offset-2"
          >
            <ArrowLeft className="mr-2 h-4 w-4" />
            Return to Homepage
          </Link>
