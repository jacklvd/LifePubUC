...
// // Define EventTempType to match your eventsTemp structure
// interface EventTempType {
//   _id: string
//   eventId: string
//   title: string
//   date: string
...
const LandingPage: React.FC = () => {
  ...
  const LandingPage = () => {
    ...
    //   setLoading(true)
    //   try {
    //     // Search using getAllEvents with search parameter
    //     const params: GetAllEventsParams = {
    //       search: searchQuery,
    //       limit: 20
    //     }

    //     const searchResults = await getAllEvents(params)
    //     setEvents(searchResults)
    //     setLoading(false)

    //     // For demo - filter mock data
    //     setTimeout(() => {
    //       // Cast eventsTemp to match Event type
    //       const tempEventsTyped = eventsTemp as unknown as Event[]

    //       // Filter by title or venue
    //       const filteredResults = tempEventsTyped.filter(event =>
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
          ...
          <div className="relative" style={{ height: '280px' }}>
            ...
            <div className="absolute bottom-3 left-1/2 transform -translate-x-1/2 flex space-x-2">
              {Array.from({ length: totalSlides }).map((_, index) => (
                <span
                  key={index}
                  className={`block h-2 w-2 rounded-full ${index === currentSlide ? 'bg-white' : 'bg-white/50'
                    }`}
                />
              ))}
            </div>
        ...
        <div className="flex overflow-x-auto pb-4 mb-6 scrollbar-thin">
          {eventTabs.map((tab, index) => (
            <div
              key={index}
              className={`px-4 py-2 whitespace-nowrap cursor-pointer ${index === 1 ? 'border-b-2 border-blue-600 text-blue-600' : ''
                }`}
            >
              {tab}
            </div>
